...
```{r}
totals_by_date <- d %>% group_by( date, item_name) %>% 
  summarise(
    amt = n(),
    total_sales = sum(item_price),
    .groups = "drop"
  ) %>% left_join( category %>% select(-n), by = "item_name")
```
```{r}
...
## Group by transaction, date, and item
